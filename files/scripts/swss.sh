...
function validate_restore_count()
{
    if [[ x"$WARM_BOOT" == x"true" ]]; then
        RESTORE_COUNT=`sonic-netns-exec $DEV sonic-db-cli STATE_DB hget "WARM_RESTART_TABLE|orchagent" restore_count`
...
{
    sonic-netns-exec $DEV sonic-db-cli $1 EVAL "
    local tables = {$2}
...
start() {
    ...
    if [[ x"$WARM_BOOT" != x"true" ]]; then
        ...
        sonic-netns-exec $DEV sonic-db-cli APPL_DB FLUSHDB
        sonic-netns-exec $DEV sonic-db-cli ASIC_DB FLUSHDB
        sonic-netns-exec $DEV sonic-db-cli COUNTERS_DB FLUSHDB
        sonic-netns-exec $DEV sonic-db-cli FLEX_COUNTER_DB FLUSHDB
